...
const publishEvent = (routingKey, data) => {
  ...
  try {
    _mqChannel.publish(
      EXCHANGE, routingKey,
      Buffer.from(JSON.stringify({ event: routingKey, data, ts: new Date().toISOString() })),
      { persistent: true }
    );
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    // ── Auto-generar código de referido para compradores ──
    if ((r || 'COMPRADOR') === 'COMPRADOR') {
      const refCode = require('crypto').randomBytes(4).toString('hex').toUpperCase();
      await pool.query(
        "INSERT INTO referrals(referrer_id,referral_code,status) VALUES($1,$2,'ACTIVO') ON CONFLICT DO NOTHING",
        [newUserId, refCode]
      ).catch(() => {});
      await pool.query(
        'INSERT INTO email_subscribers(user_id,email,name) VALUES($1,$2,$3) ON CONFLICT(email) DO NOTHING',
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (!await bcrypt.compare(req.body.password, r.rows[0].password_hash))
  ...
  try {
    ...
    const orders = await pool.query(`
      SELECT o.id, o.status, o.total, o.created_at,
        COALESCE(json_agg(json_build_object('title',p.title,'price',i.price,'quantity',i.quantity))
...
app.put('/api/profile', auth, async (req, res) => {
  try {
    ...
    if (req.user.role === 'VENDEDOR') {
      const { business_name, ruc, machine_type, machine_model, production_capacity,
              location_city, bank_name, bank_account, bank_cci, portfolio_desc } = req.body;
      await pool.query(`
        ...
        WHERE user_id=$11`,
        [business_name, ruc, machine_type, machine_model, production_capacity,
         location_city, bank_name, bank_account, bank_cci, portfolio_desc, req.user.id]);
    }
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: 'Error interno' }); }
});
...
app.post('/api/products', auth, role('VENDEDOR'), upload.single('image'), async (req, res) => {
  try {
    const { title, description, price, categoryId, minQuantity,
            productionTimeDays, requiresDesignFile, materialsAvailable, sizesAvailable } = req.body;
    const imageUrl = req.file?.location || req.file?.path
      ? `http://localhost:3001/uploads/${req.file.filename}` : null;
    await pool.query(
      ...
      [req.user.id, categoryId || null, title, description, price, minQuantity || 1,
       productionTimeDays || 3, requiresDesignFile === 'true', materialsAvailable || null,
       sizesAvailable || null, imageUrl]
    );
    res.json({ ok: true });
  } catch (err) { console.error(err); res.status(500).json({ error: 'Error interno' }); }
});
...
app.get('/api/sellers/:id', async (req, res) => {
  try {
    ...
    const products = await pool.query(
      'SELECT * FROM products WHERE seller_id=$1 AND active=TRUE ORDER BY created_at DESC',
      [req.params.id]
    );
    const reviews = await pool.query(`
...
app.get('/api/my-quotations', auth, role('COMPRADOR'), async (req, res) => {
  try {
    const r = await pool.query(`
      SELECT q.*, p.title AS product_title, p.image_url, u.name AS seller_name,
...
app.post('/api/quotations/:id/accept', auth, role('COMPRADOR'), async (req, res) => {
  try {
    const q = await pool.query(
      'SELECT * FROM quotations WHERE id=$1 AND buyer_id=$2 AND status=$3',
      [req.params.id, req.user.id, 'ENVIADA']
    );
    ...
    await pool.query(
      ...
      [ordId, quot.product_id || 1, quot.seller_id, quot.quantity,
...
app.get('/api/quotations/compare', auth, role('COMPRADOR'), async (req, res) => {
  try {
    ...
    const r = await pool.query(`
      ...
      [req.user.id, productTitle || null, `%${productTitle}%`]
    );
    res.json(r.rows);
  } catch (err) { res.status(500).json({ error: 'Error interno' }); }
});
...
app.post('/api/payments/create', auth, async (req, res) => {
  try {
    ...
    // ── Generar boleta electrónica automáticamente ──
    try {
      ...
      if (!receiptCheck.rows[0]) {
        ...
        const buyer        = await pool.query(
          'SELECT o.*, u.name AS buyer_name FROM orders o JOIN users u ON u.id=o.buyer_id WHERE o.id=$1',
          [order.id]
        );
        if (buyer.rows[0]) {
          ...
          await pool.query(
            `INSERT INTO electronic_receipts
             (order_id,receipt_number,correlative,buyer_id,buyer_name,subtotal,igv,total,status)
             VALUES($1,$2,$3,$4,$5,$6,$7,$8,'EMITIDA')`,
            [order.id, receiptNumber, corr.rows[0].n,
             bd.buyer_id, bd.buyer_name,
             subtotal.toFixed(2), igv.toFixed(2), bd.total]
          );
          await notify(bd.buyer_id, 'BOLETA_EMITIDA', '🧾 Boleta emitida',
            `Boleta ${receiptNumber} por S/ ${bd.total}`, '/buyer/receipts').catch(() => {});
        }
      }
...
app.post('/api/orders/:id/refund', auth, async (req, res) => {
  try {
    ...
    if (!['PAGADA', 'EN_PRODUCCION'].includes(o.rows[0].status))
      return res.status(400).json({ error: 'No se puede reembolsar en este estado' });
...
app.post('/api/orders/:id/cancel', auth, role('COMPRADOR'), async (req, res) => {
  try {
    const o = await pool.query('SELECT * FROM orders WHERE id=$1 AND buyer_id=$2', [req.params.id, req.user.id]);
    if (!o.rows.length) return res.status(404).json({ error: 'Orden no encontrada' });
    if (!['CREADA', 'PENDIENTE_PAGO', 'PAGADA'].includes(o.rows[0].status))
  ...
  try {
    ...
    if (o.rows[0].buyer_id !== req.user.id && req.user.role !== 'ADMIN') {
      const isSeller = await pool.query(
        'SELECT 1 FROM order_items WHERE order_id=$1 AND seller_id=$2', [req.params.id, req.user.id]
      );
...
app.post('/api/seller/payout', auth, role('VENDEDOR'), async (req, res) => {
  try {
    ...
    if (!sp.rows.length || !sp.rows[0].bank_account)
      return res.status(400).json({ error: 'Configura tu cuenta bancaria primero en tu perfil' });
    const admins = await pool.query("SELECT id FROM users WHERE role='ADMIN'");
...
app.get('/api/admin/dashboard', auth, role('ADMIN'), async (req, res) => {
  try {
    ...
    const convRate = await pool.query(`
      SELECT COUNT(*) AS total_quot, COUNT(CASE WHEN status='ACEPTADA' THEN 1 END) AS aceptadas
      FROM quotations`);
    const topSellers = await pool.query(`
      SELECT u.name,SUM(i.price) AS revenue,SUM(i.net) AS neto,COUNT(DISTINCT o.id) AS ventas,sp.rating_avg
      FROM order_items i JOIN orders o ON o.id=i.order_id
    ...
    const catStats = await pool.query(`
      ...
      JOIN categories c ON c.id=p.category_id JOIN orders o ON o.id=i.order_id
      WHERE o.status IN ('PAGADA','EN_PRODUCCION','LISTO','ENVIADA','ENTREGADA')
  ...
  try {
    ...
    if (p.rows.length) {
      await notify(p.rows[0].seller_id, 'PRODUCTO_MODERADO',
        ...
        active ? `Tu producto "${p.rows[0].title}" fue aprobado.`
               : `Tu producto "${p.rows[0].title}" fue rechazado. Motivo: ${reason}`,
        '/seller/orders');
    }
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: 'Error interno' }); }
