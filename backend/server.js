...
app.get('/migrate', async (req, res) => {
  ...
  await q('loc_manager', "ALTER TABLE locations ADD COLUMN IF NOT EXISTS manager_id INTEGER");
  await q('loc_address', "ALTER TABLE locations ADD COLUMN IF NOT EXISTS address VARCHAR(300)");
  await q('loc_phone', "ALTER TABLE locations ADD COLUMN IF NOT EXISTS phone VARCHAR(50)");
  await q('loc_email', "ALTER TABLE locations ADD COLUMN IF NOT EXISTS email VARCHAR(200)");
  await q('loc_type', "ALTER TABLE locations ADD COLUMN IF NOT EXISTS type VARCHAR(50) DEFAULT 'ALMACEN'");
  await q('tech_commission', "ALTER TABLE technicians ADD COLUMN IF NOT EXISTS commission_destination VARCHAR(200)");
  await q('tech_return', "ALTER TABLE technicians ADD COLUMN IF NOT EXISTS commission_return_date DATE");
  await q('tech_perm_type', "ALTER TABLE technicians ADD COLUMN IF NOT EXISTS permission_type VARCHAR(50)");
  await q('tech_perm_until', "ALTER TABLE technicians ADD COLUMN IF NOT EXISTS permission_until DATE");
  await q('loyalty_view', "CREATE OR REPLACE VIEW buyer_loyalty_view AS SELECT u.id AS buyer_id, u.name, u.email, COALESCE(us.score,50) AS score, COALESCE(us.color,'AMARILLO') AS traffic_light, COUNT(DISTINCT o.id) AS total_orders, COALESCE(SUM(o.total) FILTER (WHERE o.status='ENTREGADA'),0) AS total_spent, EXTRACT(MONTH FROM AGE(NOW(),u.created_at))::INT AS months_as_client, COALESCE(SUM(lp.points),0) AS total_points FROM users u LEFT JOIN user_scores us ON us.user_id=u.id LEFT JOIN orders o ON o.buyer_id=u.id LEFT JOIN loyalty_points lp ON lp.user_id=u.id WHERE u.role='COMPRADOR' GROUP BY u.id,u.name,u.email,us.score,us.color");
  await q('score_fn', "CREATE OR REPLACE FUNCTION calculate_user_score(p_user_id INTEGER) RETURNS TABLE(score INTEGER, color VARCHAR, notes TEXT) LANGUAGE plpgsql AS $fn$ DECLARE v_score INTEGER := 50; v_orders INTEGER; v_overdue INTEGER; v_months INTEGER; BEGIN SELECT COUNT(*) INTO v_orders FROM orders WHERE buyer_id=p_user_id AND status='ENTREGADA'; SELECT COUNT(*) INTO v_overdue FROM credit_installments ci JOIN credit_applications ca ON ca.id=ci.credit_id WHERE ca.buyer_id=p_user_id AND ci.status='VENCIDO'; SELECT EXTRACT(MONTH FROM AGE(NOW(),created_at))::INT INTO v_months FROM users WHERE id=p_user_id; v_score := 50+(v_orders*5)-(v_overdue*10)+LEAST(v_months,24); v_score := GREATEST(0,LEAST(100,v_score)); RETURN QUERY SELECT v_score, CASE WHEN v_score>=70 THEN 'VERDE'::VARCHAR WHEN v_score>=40 THEN 'AMARILLO'::VARCHAR ELSE 'ROJO'::VARCHAR END, ('Score: '||v_score)::TEXT; END; $fn$");
  await q('scores_seed', "INSERT INTO user_scores(user_id,score,color) SELECT id,50,'AMARILLO' FROM users WHERE role IN ('COMPRADOR','VENDEDOR') ON CONFLICT(user_id) DO NOTHING");
  res.json({ done, fail });
});
