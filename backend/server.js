...
const C = {
  bg: '#080c14', bg2: '#0d1520', bg3: '#111a28', card: '#131c2e',
  border: 'rgba(56,189,248,0.12)', borderHover: 'rgba(56,189,248,0.3)',
  blue: '#38bdf8', blueD: '#0ea5e9', blueDim: 'rgba(56,189,248,0.15)',
  green: '#34d399', greenD: '#10b981', greenDim: 'rgba(52,211,153,0.15)',
  amber: '#fbbf24', amberD: '#f59e0b', amberDim: 'rgba(251,191,36,0.15)',
  red: '#f87171', redD: '#ef4444', redDim: 'rgba(248,113,113,0.15)',
...
const URGENCY_CONFIG = {
  URGENTE: { color: C.red, bg: C.redDim, icon: '🔴', label: 'URGENTE' },
  PRONTO:  { color: C.amber, bg: C.amberDim, icon: '🟡', label: 'PRONTO' },
  OK:      { color: C.green, bg: C.greenDim, icon: '🟢', label: 'OK' },
};

const ALERT_CONFIG = {
  SIN_STOCK:     { color: C.red, bg: C.redDim, icon: '⛔', label: 'Sin Stock' },
  STOCK_BAJO:    { color: C.amber, bg: C.amberDim, icon: '⚠️', label: 'Stock Bajo' },
  STOCK_CRITICO: { color: C.purple, bg: C.purpleDim, icon: '🔶', label: 'Crítico' },
};

export default function InventoryDashboard() {
  const [tab, setTab] = useState('alerts');
  const [alerts, setAlerts] = useState({ alerts: [], summary: {} });
  const [cashFlow, setCashFlow] = useState({ cash_flow: [], inventory: [], movements: [] });
  const [prediction, setPrediction] = useState({ predictions: [], urgentes: 0, pronto: 0 });
  ...
  useEffect(() => {
    Promise.all([
      ...
      apiFetch('/api/admin/locations/cash-flow'),
      apiFetch('/api/admin/inventory/prediction'),
    ]).then(([a, c, p]) => {
      setAlerts(a);
      setCashFlow(c);
      setPrediction(p);
  ...
  const sendDailyReport = async () => {
    ...
    setReportMsg('');
    try {
      const r = await apiFetch('/api/admin/reports/daily', { method: 'POST' });
      setReportMsg(`✅ Reporte del ${r.reporte?.fecha} enviado a todos los admins`);
    } catch(e) {
      setReportMsg('❌ Error al generar reporte');
    } finally {
  ...
  const tabs = [
    { id: 'alerts', label: '🚨 Alertas Stock', count: alerts.summary?.sin_stock + alerts.summary?.stock_bajo },
    { id: 'cashflow', label: '💰 Flujo por Sede', count: cashFlow.cash_flow?.length },
    { id: 'prediction', label: '🔮 Predicción', count: prediction.urgentes },
    { id: 'report', label: '📊 Reporte Diario' },
  ];

  if (loading) return (
    <div style={{ fontFamily: "'DM Mono',monospace", background: C.bg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      <div style={{ width: 48, height: 48, border: `3px solid ${C.blue}30`, borderTop: `3px solid ${C.blue}`, borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
      <p style={{ color: C.blue, fontSize: 13, letterSpacing: 2 }}>CARGANDO SISTEMA</p>
  ...
  return (
    <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif", background: C.bg, minHeight: '100vh', color: C.text }}>
      <style>{`
        ...
        .card-hover:hover{border-color:${C.borderHover}!important;transform:translateY(-2px);transition:all 0.2s}
      ...
      <div style={{ background: C.bg2, borderBottom: `1px solid ${C.border}`, padding: '0 32px' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', display: 'flex', alignItems: 'center', height: 64, justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <a href="/admin/dashboard" style={{ color: C.textMuted, textDecoration: 'none', fontSize: 13 }}>← Panel Admin</a>
            <span style={{ color: C.border }}>|</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 32, height: 32, background: `linear-gradient(135deg,${C.blue},${C.purple})`, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>📦</div>
              <div>
                <div style={{ fontWeight: 800, fontSize: 15, color: C.text }}>Centro de Inventario</div>
                <div style={{ fontSize: 11, color: C.textMuted }}>Alertas · Flujo · Predicción · Reportes</div>
              </div>
            </div>
          </div>
          {/* Summary badges */}
          <div style={{ display: 'flex', gap: 10 }}>
            {alerts.summary?.sin_stock > 0 && (
              <div style={{ background: C.redDim, border: `1px solid ${C.red}40`, borderRadius: 20, padding: '4px 12px', fontSize: 12, color: C.red, fontWeight: 700, animation: 'pulse 2s infinite' }}>
                ⛔ {alerts.summary.sin_stock} sin stock
              </div>
            )}
            {prediction.urgentes > 0 && (
              <div style={{ background: C.amberDim, border: `1px solid ${C.amber}40`, borderRadius: 20, padding: '4px 12px', fontSize: 12, color: C.amber, fontWeight: 700 }}>
                ⚠️ {prediction.urgentes} urgentes
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1400, margin: '0 auto', display: 'flex', gap: 2 }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{ padding: '10px 20px', border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 13, fontWeight: tab === t.id ? 700 : 400, color: tab === t.id ? C.blue : C.textMuted, borderBottom: tab === t.id ? `2px solid ${C.blue}` : '2px solid transparent', display: 'flex', alignItems: 'center', gap: 6 }}>
              {t.label}
              {t.count > 0 && <span style={{ background: tab === t.id ? C.blueDim : 'rgba(255,255,255,0.05)', borderRadius: 20, padding: '1px 7px', fontSize: 11, fontWeight: 700 }}>{t.count}</span>}
            </button>
          ))}
        </div>
      </div>

      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '28px 32px', animation: 'fadeUp 0.4s ease' }}>
        ...
        {tab === 'alerts' && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 24 }}>
              {[
                { label: 'Sin Stock', value: alerts.summary?.sin_stock || 0, color: C.red, icon: '⛔', desc: 'Requiere reposición inmediata' },
                { label: 'Stock Bajo', value: alerts.summary?.stock_bajo || 0, color: C.amber, icon: '⚠️', desc: 'Por debajo del mínimo' },
                { label: 'Stock Crítico', value: alerts.summary?.stock_critico || 0, color: C.purple, icon: '🔶', desc: '50% por encima del mínimo' },
              ].map((s, i) => (
                <div key={i} className="card-hover" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: '20px 24px', position: 'relative', overflow: 'hidden', cursor: 'default' }}>
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: s.color, borderRadius: '14px 14px 0 0' }} />
                  <div style={{ fontSize: 28 }}>{s.icon}</div>
                  <div style={{ fontSize: 36, fontWeight: 900, color: s.color, margin: '8px 0 4px' }}>{s.value}</div>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 4, height: 18, background: `linear-gradient(180deg,${C.blue},${C.purple})`, borderRadius: 2 }} />
                <div style={{ fontWeight: 700, fontSize: 14 }}>Items con alertas de stock</div>
                <div style={{ marginLeft: 'auto', fontSize: 12, color: C.textMuted }}>{alerts.alerts?.length || 0} items</div>
              </div>
              {alerts.alerts?.length === 0 ? (
                <div style={{ padding: '60px 20px', textAlign: 'center' }}>
                  <div style={{ fontSize: 48, marginBottom: 12 }}>✅</div>
                  <p style={{ color: C.green, fontWeight: 700, fontSize: 15 }}>Todos los stocks están bien</p>
                  <p style={{ color: C.textMuted, fontSize: 13 }}>No hay alertas activas en este momento</p>
                </div>
              ) : (
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead>
                    <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                      {['Alerta','Item','Sede','Stock Actual','Stock Mínimo','Valor','Acción'].map(h => (
                        <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: 11, color: C.textMuted, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {alerts.alerts?.map((item, i) => {
                      const cfg = ALERT_CONFIG[item.alert_level] || ALERT_CONFIG.STOCK_BAJO;
                      return (
                        <tr key={i} className="row-hover" style={{ borderBottom: `1px solid rgba(255,255,255,0.03)` }}>
                          <td style={{ padding: '12px 16px' }}>
                            <span style={{ background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.color}30`, borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>
                              {cfg.icon} {cfg.label}
                            </span>
                          </td>
                          <td style={{ padding: '12px 16px', fontWeight: 700 }}>{item.item_name}</td>
                          <td style={{ padding: '12px 16px', color: C.textDim }}>{item.location_name} · {item.city}</td>
                          <td style={{ padding: '12px 16px' }}>
                            <span style={{ color: cfg.color, fontWeight: 900, fontSize: 15 }}>{item.quantity}</span>
                            <span style={{ color: C.textMuted, fontSize: 11 }}> uds</span>
                          </td>
                          <td style={{ padding: '12px 16px', color: C.textMuted }}>{item.min_quantity} uds</td>
                          <td style={{ padding: '12px 16px', color: C.green, fontWeight: 700 }}>{fmt(item.quantity * item.unit_cost)}</td>
                          <td style={{ padding: '12px 16px' }}>
                            <a href={`/admin/inventory?location=${item.location_id}`}
                              style={{ background: C.blueDim, color: C.blue, border: `1px solid ${C.blue}30`, borderRadius: 8, padding: '5px 12px', fontSize: 11, fontWeight: 700, textDecoration: 'none' }}>
                              Reponer →
                            </a>
                          </td>
                        </tr>
        ...
        {tab === 'cashflow' && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 16, marginBottom: 24 }}>
              {cashFlow.cash_flow?.map((sede, i) => {
                const colors = [C.blue, C.green, C.amber, C.purple, C.red];
                const c = colors[i % colors.length];
                return (
                  <div key={sede.id} className="card-hover" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20, position: 'relative', overflow: 'hidden' }}>
                    <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: 4, background: c, borderRadius: '14px 0 0 14px' }} />
                    <div style={{ paddingLeft: 12 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                        <div>
                          <div style={{ fontWeight: 800, fontSize: 16, color: C.text }}>{sede.sede}</div>
                          <div style={{ fontSize: 12, color: C.textMuted }}>{sede.city}</div>
                        </div>
                        <div style={{ background: `${c}15`, border: `1px solid ${c}30`, borderRadius: 8, padding: '3px 10px', fontSize: 11, color: c, fontWeight: 700 }}>
                          {sede.vendedores_activos} vendedores
                        </div>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        <div style={{ background: C.bg3, borderRadius: 8, padding: '10px 12px' }}>
                          <div style={{ fontSize: 10, color: C.textMuted, marginBottom: 3 }}>REVENUE</div>
                          <div style={{ fontSize: 16, fontWeight: 900, color: c }}>{fmt(sede.revenue_total)}</div>
                        </div>
                        <div style={{ background: C.bg3, borderRadius: 8, padding: '10px 12px' }}>
                          <div style={{ fontSize: 10, color: C.textMuted, marginBottom: 3 }}>COMISIÓN</div>
                          <div style={{ fontSize: 16, fontWeight: 900, color: C.amber }}>{fmt(sede.comision_futura)}</div>
                        </div>
                        <div style={{ background: C.bg3, borderRadius: 8, padding: '10px 12px' }}>
                          <div style={{ fontSize: 10, color: C.textMuted, marginBottom: 3 }}>NETO</div>
                          <div style={{ fontSize: 16, fontWeight: 900, color: C.green }}>{fmt(sede.neto_vendedores)}</div>
                        </div>
                        <div style={{ background: C.bg3, borderRadius: 8, padding: '10px 12px' }}>
                          <div style={{ fontSize: 10, color: C.textMuted, marginBottom: 3 }}>ÓRDENES</div>
                          <div style={{ fontSize: 16, fontWeight: 900, color: C.text }}>{sede.total_ordenes}</div>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 4, height: 18, background: `linear-gradient(180deg,${C.green},${C.blue})`, borderRadius: 2 }} />
                <div style={{ fontWeight: 700, fontSize: 14 }}>Inventario valorizado por sede</div>
              </div>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                    {['Sede','Items','Stock Total','Valor Inventario','Valor Potencial','Stock Bajo'].map(h => (
                      <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: 11, color: C.textMuted, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {cashFlow.inventory?.map((inv, i) => (
                    <tr key={i} className="row-hover" style={{ borderBottom: `1px solid rgba(255,255,255,0.03)` }}>
                      <td style={{ padding: '12px 16px', fontWeight: 700 }}>{inv.sede}</td>
                      <td style={{ padding: '12px 16px', color: C.textDim }}>{inv.items}</td>
                      <td style={{ padding: '12px 16px', color: C.blue, fontWeight: 700 }}>{inv.stock_total} uds</td>
                      <td style={{ padding: '12px 16px', fontWeight: 800, color: C.text }}>{fmt(inv.valor_inventario)}</td>
                      <td style={{ padding: '12px 16px', color: C.green }}>{fmt(inv.valor_venta_potencial)}</td>
                      <td style={{ padding: '12px 16px' }}>
                        {parseInt(inv.items_bajo_stock) > 0 ? (
                          <span style={{ background: C.amberDim, color: C.amber, borderRadius: 20, padding: '2px 10px', fontSize: 11, fontWeight: 700 }}>⚠️ {inv.items_bajo_stock}</span>
                        ) : (
                          <span style={{ background: C.greenDim, color: C.green, borderRadius: 20, padding: '2px 10px', fontSize: 11, fontWeight: 700 }}>✅ OK</span>
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        {tab === 'prediction' && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 24 }}>
              {[
                { label: 'Urgente (≤7 días)', value: prediction.urgentes, color: C.red, icon: '🔴', desc: 'Se acaban esta semana' },
                { label: 'Pronto (≤15 días)', value: prediction.pronto, color: C.amber, icon: '🟡', desc: 'Revisar en 2 semanas' },
                { label: 'Sin datos de consumo', value: prediction.predictions?.filter(p => parseFloat(p.consumo_diario) === 0).length, color: C.textMuted, icon: '⚪', desc: 'Sin movimientos recientes' },
              ].map((s, i) => (
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 4, height: 18, background: `linear-gradient(180deg,${C.purple},${C.blue})`, borderRadius: 2 }} />
                <div style={{ fontWeight: 700, fontSize: 14 }}>🔮 Predicción de agotamiento</div>
                <div style={{ marginLeft: 'auto', fontSize: 12, color: C.textMuted }}>Basado en consumo últimos 30 días</div>
              </div>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead>
                    <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                      {['Urgencia','Item','Sede','Stock','Consumo/día','Días hasta vacío','Días hasta mínimo','Pedido sugerido'].map(h => (
                        <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: 10, color: C.textMuted, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, whiteSpace: 'nowrap' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {prediction.predictions?.sort((a,b) => a.dias_hasta_vacio - b.dias_hasta_vacio).map((p, i) => {
                      const cfg = URGENCY_CONFIG[p.urgencia];
                      return (
                        <tr key={i} className="row-hover" style={{ borderBottom: `1px solid rgba(255,255,255,0.03)` }}>
                          <td style={{ padding: '12px 16px' }}>
                            <span style={{ background: cfg.bg, color: cfg.color, borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>{cfg.icon} {cfg.label}</span>
                          </td>
                          <td style={{ padding: '12px 16px', fontWeight: 600 }}>{p.item_name}</td>
                          <td style={{ padding: '12px 16px', color: C.textDim, fontSize: 12 }}>{p.sede}</td>
                          <td style={{ padding: '12px 16px', fontWeight: 700, color: C.blue }}>{p.stock_actual}</td>
                          <td style={{ padding: '12px 16px', color: C.textDim }}>{p.consumo_diario}/día</td>
                          <td style={{ padding: '12px 16px' }}>
                            <span style={{ color: cfg.color, fontWeight: 800 }}>{p.dias_hasta_vacio === 999 ? '∞' : `${p.dias_hasta_vacio}d`}</span>
                          </td>
                          <td style={{ padding: '12px 16px', color: C.textDim }}>{p.dias_hasta_minimo === 999 ? '∞' : `${p.dias_hasta_minimo}d`}</td>
                          <td style={{ padding: '12px 16px' }}>
                            {p.pedido_sugerido > 0 ? (
                              <span style={{ background: C.blueDim, color: C.blue, borderRadius: 8, padding: '3px 10px', fontWeight: 700 }}>+{p.pedido_sugerido} uds</span>
                            ) : (
        ...
        {tab === 'report' && (
          <div style={{ maxWidth: 640, margin: '0 auto' }}>
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 32, textAlign: 'center' }}>
              ...
              <div style={{ fontWeight: 800, fontSize: 22, marginBottom: 8 }}>Reporte Diario Futura</div>
              <div style={{ color: C.textMuted, fontSize: 14, marginBottom: 32, lineHeight: 1.6 }}>
                Genera un resumen completo del día con ventas, stock, técnicos activos y créditos pendientes. Se enviará como notificación a todos los administradores.
              </div>

              {reportMsg && (
                <div style={{ background: reportMsg.startsWith('✅') ? C.greenDim : C.redDim, border: `1px solid ${reportMsg.startsWith('✅') ? C.green : C.red}30`, borderRadius: 10, padding: '12px 20px', marginBottom: 24, color: reportMsg.startsWith('✅') ? C.green : C.red, fontWeight: 600 }}>
                  {reportMsg}
                </div>
              )}

              <button onClick={sendDailyReport} disabled={reportLoading}
                style={{ background: `linear-gradient(135deg,${C.blue},${C.blueD})`, color: 'white', border: 'none', borderRadius: 12, padding: '14px 32px', fontSize: 15, fontWeight: 700, cursor: reportLoading ? 'not-allowed' : 'pointer', opacity: reportLoading ? 0.7 : 1, width: '100%', marginBottom: 16 }}>
                {reportLoading ? '⏳ Generando reporte...' : '📤 Generar y Enviar Reporte'}
              </button>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 24 }}>
                {[
                  { icon: '💰', label: 'Ventas del día', desc: 'Órdenes y revenue' },
                  { icon: '📦', label: 'Estado de stock', desc: 'Alertas activas' },
                  { icon: '🔧', label: 'Técnicos', desc: 'Activos hoy' },
                  { icon: '💳', label: 'Créditos', desc: 'Solicitudes pendientes' },
                ].map((item, i) => (
                  <div key={i} style={{ background: C.bg3, border: `1px solid ${C.border}`, borderRadius: 10, padding: '14px 16px', textAlign: 'left' }}>
