...
const { redis } = require('../../shared/middleware');
...
const sendMessage = async (to, text) => {
  if (!WA_PHONE_ID || !WA_TOKEN_API) {
    ...
    await pool.query(
      'INSERT INTO whatsapp_messages(phone,direction,message,timestamp) VALUES($1,$2,$3,now()) ON CONFLICT DO NOTHING',
      [to, 'OUT', text]
    ).catch(() => {});
    return { mock: true };
  }
...
const processMessage = async (phone, message, userName) => {
  ...
  switch (state.state) {
    ...
    case STATES.VIEW_PRODUCT: {
      ...
      if (!isNaN(idx) && context.searchResults && context.searchResults[idx]) {
        ...
        await sendButtons(phone,
          `*${p.title}*\n💰 S/ ${parseFloat(p.price).toLocaleString()}\n👤 ${p.seller_name}`,
          ['Solicitar cotización', 'Ver en web', 'Buscar otro']
        );
        state.state = STATES.QUOTE_SPECS;
      } else {
        await sendMessage(phone, 'Escribe el número del producto que te interesa.');
      }
      break;
    }
    ...
    case STATES.QUOTE_SPECS: {
      context.quoteSpec = message;
      state.state = STATES.QUOTE_QUANTITY;
      await sendMessage(phone, TEMPLATES.quoteQty(message));
      break;
    }
  ...
  try {
    ...
    if (value?.messages?.length) {
      for (const msg of value.messages) {
        ...
        await pool.query(
          'INSERT INTO whatsapp_messages(phone,direction,message,timestamp) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING',
...
app.post('/api/whatsapp/test', async (req, res) => {
  ...
  // Process FSM — bot responses get saved via sendMessage mock
  await processMessage(phone, message, 'Test User');

  // Return full conversation history
  const msgs = await pool.query(
    'SELECT * FROM whatsapp_messages WHERE phone=$1 ORDER BY timestamp ASC LIMIT 40',
    [phone]
  ).catch(() => ({ rows: [] }));
